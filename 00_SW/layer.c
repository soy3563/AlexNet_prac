    ...
    for (int oc = 0; oc < out_c; ++oc) {
        for (int oy = 0; oy < out_h; ++oy) {
            for (int ox = 0; ox < out_w; ++ox) {
                            ...
                            for (int kx = 0; kx < ksize; ++kx) {
                                ...
                                if (ix >= 0 && iy >= 0 && ix < in_w && iy < in_h) {
                                    ...
                                    int k_idx = oc * in_c * ksize * ksize + ic * ksize * ksize + ky * ksize + kx;
                                        int16_t product = (int16_t)input[in_idx] * (int16_t)weight[k_idx];  // 8bit intermediate
                                        sum += product;
                                }
                            }
                        }
                    ...
                    int16_t temp = sum + bias[oc];
                    if (temp < -8) temp = -8;
                    else if (temp > 7) temp = 7;
                    output[out_idx] = (int8_t)temp;
                }
            }
        }
}
...
void fc_layer(int8_t* input, int8_t* output, int8_t* weight, int8_t* bias, int in_size, int out_size) {
    for (int i = 0; i < out_size; ++i) {
        int16_t sum = (int16_t)bias[i];
        for (int j = 0; j < in_size; ++j) {
            sum += (int16_t)input[j] * (int16_t)weight[i * in_size + j];
        }
        if (sum < -8) sum = -8;
        else if (sum > 7) sum = 7;
        output[i] = (int8_t)sum;
...
void write_layer_data(const char* layer_name, int8_t* weight, int weight_size, int8_t* bias, int bias_size) {
    ...
    snprintf(wfile, sizeof(wfile), "./data/%s_weight.txt", layer_name);
    snprintf(bfile, sizeof(bfile), "./data/%s_bias.txt", layer_name);
...
void write_output_data(const char* layer_name, int8_t* output, int size) {
    char fname[64];
    snprintf(fname, sizeof(fname), "./data/%s_out.txt", layer_name);
